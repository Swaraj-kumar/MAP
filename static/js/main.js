document.addEventListener('DOMContentLoaded', () => {
    ...
    function createStateMap(stateData, plants, stateName) {
        ...
        // Get districts with plants
        const districtsWithPlants = new Set(Object.keys(plantsByDistrict));
    
        // Prepare districts data with plant counts
        ...
        currentChart = Highcharts.mapChart('map-container', {
            chart: {
                ...
                events: {
                    load: function() {
                        // Create a custom legend item for highlighting districts with plants
                        const chart = this;
                        const legendGroup = chart.legend.group;
                        const legendItem = chart.renderer.g('legend-item-plants')
                            .add(legendGroup);
    
                        // Add hover events to the legend item
                        legendItem
                ...
                series: {
                    events: {
                        legendItemClick: function(e) {
                            if (this.type === 'map') {
                                if (this.visible) {
                                    // Show only districts with plants and highlight them
    ...
    function showPlantDetails(plant) {
        ...
        // Create a formatted list of all plant details
        const details = Object.entries(plant)
            .filter(([key, value]) => value && value !== 'N/A' && value !== '')
